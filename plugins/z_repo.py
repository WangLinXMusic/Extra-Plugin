...
async def help(client: Client, message: Message):
    await message.reply_photo(
        photo=f"https://envs.sh/nzf.jpg",
        caption=f"""ᴄʟɪᴄᴋ ᴛᴏ ᴊᴏɪɴ ᴄʜᴀᴛ""",
        reply_markup=InlineKeyboardMarkup(
            [
                [
                    InlineKeyboardButton(
                        "sᴜᴘᴘᴏʀᴛ", url=f"https://t.me/seriousvs_version10"
                    )
                ]
            ]
        ),
...
async def add_allbot(client, message):
    ...
    if len(command_parts) != 2:
        await message.reply(
            "**⚠️ ɪɴᴠᴀʟɪᴅ ᴄᴏᴍᴍᴀɴᴅ ғᴏʀᴍᴀᴛ. ᴘʟᴇᴀsᴇ ᴜsᴇ ʟɪᴋᴇ » `/gadd @WangLin_2K5`**"
        )
        return
